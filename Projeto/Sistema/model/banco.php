...
class Banco extends PDO{
	...
	function validaUsuario($usuario, $senha) {
		...
		$sql = "SELECT id, nome, email, senha, tipo FROM " . $this->_tabela . " WHERE " . $cS . " email = '" . $nusuario . "' LIMIT 1";
		...
		if (crypt($nsenha, $nhash) === $nhash) {
		    ...
		    $_SESSION['usuarioTipo']  = $resultado[0]['tipo'];
	...
	function expulsaVisitante($erro = '', $pagina = ''){
		...
		if('login' == $pagina){
			...
			header("Location: ../view/index.php?erro=". $erro);
		}else{
